  ...
  export default {
    ...
    methods: {
      ...
      selectExpress (servicioExpress) {
        console.log('servicio expres seleccionado, users', this.usuariosBook)
        if (this.usuariosBook.length > 0) {
          if (this.role === 2) {
            console.log('es a terceros y admoin, cambiar container central')
            ...
            this.$store.dispatch('Booking/set_selectedExpress', {selectedExpress: true}) // variable hace que se levante el modal con los detales de reserva, y poder realizarla
            this.$store.dispatch('Booking/set_servicioExpress', {servicioExpress: servicioExpress})
          }
        } else {
          this.$swal({
